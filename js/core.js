  ...
  const RULES = {
    omok: {
        ...
        <ul>
            ...
            <li><strong>금수 클릭 시 패널티:</strong> 바둑판 흔들림 효과, 불협화음 사운드, 해당 위치 1초간 붉은 경고 표시</li>
        </ul>
        <h3>턴 타이머</h3>
        <ul>
    ...
    blackjack: {
        ...
        <h3>PVP 레이드 (보스 레이드) 🚧 공사 중</h3>
        <ul>
            <li>다수의 플레이어가 AI 딜러를 상대로 <strong>합산된 딜러 하트</strong>를 함께 깎는 협동 모드입니다.</li>
            <li><strong>전원 준비:</strong> 방 안의 모든 유저(인간+봇)가 준비해야 게임이 시작됩니다.</li>
        </ul>
        <h3>하트 증감</h3>
        <ul>
    ...
    tictactoe: {
        ...
        <h3>턴 타이머 — 터보 모드</h3>
        <ul>
            <li>한 턴당 기본 <strong>15초</strong> 제한. <strong>매 수마다 0.5초씩 감소</strong> (최소 1.5초).</li>
            <li>초과 시 시간 초과 패배. 3초 미만 시 긴박한 피아노 저음과 빨간 배경 깜빡임으로 경고.</li>
        </ul>
        <h3>리매치</h3>
        <ul>
    ...
    sevenpoker: {
        ...
        <h3>참가비 (Ante)</h3>
        <ul>
            <li>라운드 시작 시 별 1개 이상인 모든 플레이어에서 <strong>별 1개 차감</strong> → 팟</li>
            <li><strong>0별 올인:</strong> 참가비 지불 후 별이 0개가 되어도 해당 라운드 끝까지 카드 지급·승부 가능</li>
        </ul>
        <h3>카드 분배 (3~7구)</h3>
        <ul>
        ...
        <ul>
            ...
            <li>쇼다운 시 각 유저의 별 변동량(DeltaStars, 예: +5, -1) 표시.</li>
        </ul>
        <h3>📋 포커 족보 순서</h3>
    ...
    holdem: {
        ...
        <ul>
            <li>최대 4인, 각 플레이어는 <strong>별(⭐)×10개</strong>로 시작합니다.</li>
            <li>캐주얼한 <strong>별 서바이벌 룰</strong>을 적용합니다.</li>
        </ul>
        <h3>참가비 (Ante)</h3>
        <ul>
            <li>라운드 시작 시 별 1개 이상인 모든 플레이어에서 <strong>별 1개 차감</strong> → 팟</li>
    ...
    thief: {
        ...
        <h3>턴 진행</h3>
        <ul>
            <li>내 차례일 때 <strong>다음 생존 플레이어</strong>의 패에서 카드 1장을 뽑아 옵니다.</li>
            <li><strong>카드 뽑기 (오조작 방지):</strong> 상대 카드를 클릭하여 선택(강조 표시) → <strong>🃏 카드 뽑기</strong> 버튼 클릭. 선택 후 버튼 방식으로 잘못된 클릭 방지.</li>
    ...
    onecard: {
        ...
        <ul>
            ...
            <li><strong>컬러 조커(C)</strong>: +7장 (빨간색 시각적 강조)</li>
        ...
        <ul>
            ...
            <li>누군가 카드를 뽑거나 내서 1장 상황이 해소되면 버튼 <strong>자동 비활성화</strong>.</li>
        </ul>
        <h3>서바이벌 룰 (게임 종료)</h3>
        <ul>
            <li><strong>4인</strong>: 2명 탈출 또는 2명 파산(2승 2패) 시 종료.</li>
            <li><strong>3인</strong>: 1명 탈출 또는 2명 파산(1승 2패) 시 종료.</li>
            <li><strong>2인</strong>: 1명 탈출 또는 1명 파산 시 종료.</li>
            <li>등수별 전적: 탈출 성공자·파산하지 않은 생존자 → <strong>승리</strong>, 파산자 → <strong>패배</strong>.</li>
        </ul>
        <h3>파산</h3>
        <ul>
    ...
    alkkagi: {
        ...
        <h3>게임 개요</h3>
        <ul>
            <li><strong>2인 대전</strong>. 흑돌 5개 vs 백돌 5개. Matter.js 물리 엔진 기반.</li>
            <li>전원 Ready 시 흑/백이 무작위 배정되며, 흑이 선공입니다.</li>
        </ul>
        <h3>배치 페이즈 (20초)</h3>
        <ul>
            <li><strong>20초</strong> 안에 자기 영역(15×15 격자)에 돌 5개를 배치하세요.</li>
            <li>흑: 아래쪽 5줄(10~14행), 백: 위쪽 5줄(0~4행).</li>
            <li>시간 종료 시 미배치 돌은 기본 위치에 자동 배치됩니다.</li>
        </ul>
        <h3>대전 (체스/장기 모드)</h3>
        <ul>
            <li>내 차례에 돌을 드래그하여 당긴 뒤 놓으면 쏘아집니다.</li>
            <li><strong>기물별 질량·각도 제한</strong>: 체스/장기 모드에서 기물 타입별 물리 파라미터 적용.</li>
            <li>강하게 쏠수록 각도에 미세한 랜덤 오차가 발생합니다.</li>
        </ul>
        <h3>승리·패배 조건</h3>
        <ul>
            <li>상대 알을 모두 보드 밖으로 밀어내면 승리!</li>
            <li><strong>모든 돌이 낙하</strong>하면 해당 플레이어 패배.</li>
        </ul>
        <h3>사운드·심리전</h3>
        <ul>
            <li>The Table 특유의 <strong>피아노 사운드 인터랙션</strong>으로 튕김·충돌 시 긴장감 연출.</li>
        </ul>`
    },
    tetris: {
        title: '🧱 다인용 테트리스 (Shared Tetris) 룰',
        html: `
        <h3>게임 개요</h3>
        <ul>
            <li><strong>4인 공유 그리드 난투</strong>. 15×20 공유 보드에서 각자 블럭을 동시에 조작합니다.</li>
            <li><strong>선점형 그리드 충돌:</strong> 다른 플레이어의 블럭·굳은 블록과 <strong>겹치지 않게(비중첩)</strong> 이동해야 합니다.</li>
            <li>2명 이상 Ready 시 게임이 시작됩니다. (🚧 공사 중)</li>
        </ul>
        <h3>조작</h3>
        <ul>
            <li>방향키 또는 WASD: 좌/우/아래 이동, 위(W): 회전</li>
            <li>스페이스: 하드 드롭 (바로 고정)</li>
        </ul>
        <h3>득점 — 줄 완성자 점수 스틸</h3>
        <ul>
            <li>블럭을 고정했을 때 가로 줄이 완성되면, <strong>그 줄을 완성한 유저</strong>에게 점수가 부여됩니다.</li>
            <li>완성된 줄은 지워지고 위 블럭이 내려옵니다.</li>
        </ul>
        <h3>게임 오버</h3>
        <ul>
            <li>새 블럭을 스폰할 공간이 없으면 게임 종료.</li>
        </ul>`
    },
    duel: {
        title: '⚔️ 서부의 결투 (Western Duel) 룰',
        html: `
        <h3>게임 개요</h3>
        <ul>
            <li><strong>2인 반응 속도 테스트</strong>. Draw 신호 후 가장 빨리 클릭한 쪽이 승리!</li>
            <li>2명 입장 후 Ready 시 게임이 시작됩니다. (🚧 공사 중)</li>
        </ul>
        <h3>진행</h3>
        <ul>
            <li>준비 페이즈: 화면이 세피아로 변합니다.</li>
            <li>Draw: 신호와 함께 화면이 밝아집니다. 이때 시계를 클릭하세요!</li>
            <li><strong>ms 단위 승패 결정:</strong> 가장 빠른 반응 속도(밀리초)가 승리합니다.</li>
        </ul>
        <h3>실격 (Foul)</h3>
        <ul>
            <li><strong>신호 전 사격:</strong> Draw 신호 이전에 클릭하면 <strong>실격(Foul)</strong> 처리. 상대 승리.</li>
        </ul>`
    },
    suika: {
        title: '🍉 수박게임 (Suika Merge) 룰',
        html: `
        <h3>게임 개요</h3>
        <ul>
            <li><strong>실시간 쿨타임(3초) 충전식 투하</strong>. 3초마다 과일이 1개씩 충전되며, 최대 2개까지 저장됩니다.</li>
            <li>충전된 과일로 컨테이너에 투하하여 지분을 획득하세요! Matter.js 물리 엔진 기반 자유 낙하.</li>
            <li>2명 이상 Ready 시 게임이 시작됩니다.</li>
        </ul>
        <h3>지분 기반 점수 배분</h3>
        <ul>
            <li>모든 과일 객체에 <strong>ownerEquity</strong> (지분율) 맵이 적용됩니다.</li>
            <li>동일 크기 과일이 합쳐질 때: <strong>NewEquity[user] = (ParentA + ParentB) / 2</strong> — 합체 시마다 이전 지분 50% 희석.</li>
            <li>진화 점수 발생 시, 지분 비율에 따라 각 플레이어 점수판에 실시간 반영됩니다.</li>
        </ul>
        <h3>2^n 점수 체계</h3>
        <ul>
            <li>체리(2) → 딸기(4) → 포도(8) → … → 수박(2048). 합체 시 해당 레벨 점수를 지분 비율대로 배분.</li>
        </ul>
        <h3>호스트 기반 물리 동기화</h3>
        <ul>
            <li>방의 첫 번째 플레이어가 Host. Host 클라이언트가 Matter.js 연산 결과를 100ms마다 서버에 sync_all로 전송.</li>
            <li>다른 클라이언트는 sync_all 수신 시 과일 위치를 보정합니다.</li>
        </ul>
        <h3>투하 금지선·게임오버</h3>
        <ul>
            <li>상단 50px 영역은 투하 금지선. 과일이 3초 이상 넘어가면 게임 종료, 마지막 투하 유저에게 패널티.</li>
        </ul>`
    }
  };
        ...
        try {
          ...
          switch (parsed.type) {
            ...
            case 'alkkagi_flick':
              ...
              break;
            case 'duel_draw':
              addLog('duel-draw', raw);
              if (typeof window.handleDuelDraw === 'function') window.handleDuelDraw(parsed.data);
              break;
            ...
            case 'suika_drop_result':
              ...
              break;
            case 'suika_sync_all':
              if (typeof window.suikaOnSyncAll === 'function') window.suikaOnSyncAll(parsed.bodies || []);
              break;
  ...
  function clearAllGameContainers() {
    ...
    if (typeof window.clearDuel === 'function') window.clearDuel();
  }

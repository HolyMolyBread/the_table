...
**The Table**은 블랙잭, 텍사스 홀덤, 마작 등 다양한 보드게임 룰을 플러그인 형태로 교체할 수 있는 **범용 멀티플레이어 보드게임 엔진 플랫폼**이다. 순수 유저 간의 대결(PVP)과 AI를 상대로 한 혼자만의 도전(PVE)을 완벽히 분리하여 제공하는 것이 핵심 설계 원칙이다.
...
| **클라이언트** | 브라우저 HTML/JS | 테스트 콘솔 (`index.html`). **Matter.js** 물리 엔진 활용(알까기, 수박게임). 필요 시 3D 렌더링(Three.js) 추가 도입 예정 |
...
├── css/
│   └── style.css            # 전역 스타일 (인라인 CSS 분리)
├── js/
│   ├── core.js              # 웹소켓, 상태 관리, 공통 UI 로직 (Supabase Auth, 채팅, 전적 등)
│   └── games/               # 개별 게임 클라이언트 로직
│       ├── omok.js          # 오목 보드·렌더링·클릭 이벤트
│       ├── blackjack.js     # 블랙잭 UI
│       ├── tictactoe.js     # 틱택토 UI
│       ├── connect4.js      # 4목 UI
│       ├── holdem.js        # 텍사스 홀덤 UI
│       ├── sevenpoker.js    # 세븐 포커 UI
│       ├── indian.js        # 인디언 포커 UI
│       ├── thief.js         # 도둑잡기 UI
│       ├── onecard.js       # 원카드 UI
│       ├── mahjong.js       # 마작 UI
...
| `alkkagi` | 알까기 (2인, Matter.js) | `NewAlkkagiGame()` |
| `tetris` | 다인용 테트리스 (4인) | `NewTetrisGame()` |
| `duel` | 서부의 결투 (2인) | `NewDuelGame()` |
| `suika` | 수박게임 (Matter.js, 지분 점수) | `NewSuikaGame()` |
| 그 외 | — (채팅방) | `nil` |
...
| 게임 | 파일 (예정) | 상태 | 비고 |
|---|---|---|---|
| 블랙잭 | `plugin_blackjack.go` | **완료** | PVE 딜러 AI. **PVP 레이드** (🚧 공사 중): 다수 플레이어 vs AI 딜러 협동 모드, 전원 준비 시스템. 접두사: `blackjack` |
| 텍사스 홀덤 | `plugin_holdem.go` | **완료** | 별(⭐)×10 서바이벌. **참가비(Ante)** 라운드당 별 1개. **0별 올인** 시 참가비 지급 후 별이 0이 되어도 해당 라운드 끝까지 카드 지급·승부 가능. DeltaStars 표시. 접두사: `holdem` |
| 세븐 포커 | `plugin_sevenpoker.go` | **완료** | 3~7구 분배, 히든 카드. **참가비(Ante)** 라운드당 별 1개. **0별 올인** 시 해당 라운드 끝까지 카드 지급·승부 가능. 접두사: `sevenpoker` |
| 인디언 포커 | `plugin_indian.go` | **완료** | 하트 서바이벌(❤️×10), 개별 상태 전송(뒷면/앞면), 선공/포기 -1, 승부/패 -2+2, 접두사: `indian` |
| 도둑잡기 | `plugin_thief.go` | **완료** | 2~4인, 53장(52+조커) 페어 제거. **카드 선택(강조)→뽑기 버튼 클릭** 방식으로 오조작 방지. 접두사: `thief` |
| 원카드 | `plugin_onecard.go` | **완료** | 2~4인, 탑 카드 매칭, J=스킵 Q=방향반전. **서바이벌**: 4인(2승2패), 3인(1승2패), 2인(1승1패) 종료. 탈출·생존=승, 파산=패. C_JOKER 빨간색 강조, 원카드! 버튼 자동 비활성화. 접두사: `onecard` |

### 🀄 타일/그리드 게임

| 게임 | 파일 (예정) | 상태 | 비고 |
|---|---|---|---|
| 마작 | `plugin_mahjong.go` | **Phase 1 완료** | 🀄 리치 마작 기반. 패 분배·쯔모·타패 구현. **4인 필수** (2~3인 모드 향후 예정). 치/퐁/깡·역 계산 대기 중. 로비 카드에 🚧 공사 중 배지. 접두사: `mahjong` |
| 오목 | `plugin_gomoku.go` | **완료** | 15×15, **렌주룰(3-3, 4-4, 장목 금수)** 공식 적용, 금수 클릭 시 패널티(흔들림/불협화음). 15초 타이머, 리매치 지원. 접두사: `omok` |
| 4목 (Connect 4) | `plugin_connect4.go` | **완료** | 6×7, 중력 낙하, 가로·세로·대각선 4목 승리, 무승부 판정. 승무패 전적 기록. 접두사: `connect4` |
| 틱택토 | `plugin_tictactoe.go` | **완료** | 3×3, **터보 모드**(매 수마다 제한 시간 0.5초 감소, 최소 1.5초). 승무패 전적 기록. 접두사: `tictactoe` |
| 다인용 테트리스 | `plugin_tetris.go` | **🚧 공사 중** | 4인 공유 그리드 난투. **선점형 그리드 충돌(비중첩)**, 줄 완성자 점수 스틸. 접두사: `tetris` |
...
| 게임 | 파일 (예정) | 상태 | 비고 |
|---|---|---|---|
| 알까기 | `plugin_alkkagi.go` | **🚧 공사 중** | Matter.js 기반. **배치 페이즈(20초)**, 5개 기물, 체스/장기 모드(질량·각도 제한), 모든 돌 낙하 시 패배. 접두사: `alkkagi` |
| 수박게임 | `plugin_suika.go` | **완료** | Matter.js 기반. **호스트 기반 물리 동기화**, 3초 쿨타임 충전식 투하, **지분 기반 2^n 점수**. 접두사: `suika` |
| 서부의 결투 | `plugin_duel.go` | **🚧 공사 중** | 반응속도 테스트. **신호 전 사격 시 실격(Foul)**, ms 단위 승패 결정. 접두사: `duel` |
...
| **공사 중 배지** | 블랙잭 레이드, 알까기, 테트리스, 서부의 결투, 마작(4인/3인) 카드에 🚧 공사 중 배지 표시 |
| **Matter.js 물리 엔진** | 알까기·수박게임 등 물리 기반 게임에 활용. 호스트 기반 동기화로 Desync 방지 |
| **가속 타이머 시스템** | 틱택토 터보 모드(매 수 0.5초 감소). 3초 미만 시 피아노 저음·빨간 깜빡임 연출 |
| **피아노 사운드·심리전** | The Table 특유의 `SoundManager.playPianoNote` 인터랙션. 인디언 포커·도둑잡기 등 심리전 강조 |
...
| **Phase 8.4** | **마작 (Mahjong) Phase 1** — 136장 패 분배, 쯔모/타패 기본 뼈대, 4인 원탁 UI, 봇 기본 대응. **4인 플레이 강제** (2~3인 모드는 향후 별도 패치 예정) | 완료 |
...
| **Phase 8.7** | **프론트엔드 모놀리식 구조 분리 및 최적화** — `index.html` 인라인 JS/CSS를 개별 파일로 분리, 안전한 정적 파일 라우팅 적용 | 완료 |
| **Phase 8.x** | **현재 이슈 수정 및 안정화** — (1) DB 전적 저장 버그 픽스: JWT 파이프라인·Supabase RLS·`on_conflict` 로직으로 전적 영구 저장. (2) UI/UX 스크롤 잘림 해결: Flexbox 압착 방지, `margin: auto 0`, `flex-start` 적용. (3) 텍사스 홀덤: 매 턴 이동하는 딜러(D) 배지 마커. (4) 도둑잡기: 타겟 시계방향 고정, 2인 마주보게(Top) 배치, 3~4인 사이드 상향(Top 35%)로 클릭 간섭 해결. (5) 원카드: 덱 하이라이트 애니메이션, 핸드 렌더링 Optional Chaining 방어. (6) 인디언 포커: 포기 시 양측 카드 공개 오버레이 | 완료 |
| **Phase 8.y** | **게임 규칙 고도화 및 플랫폼 UX** — (1) 원카드 서바이벌 룰: 4인 2탈출/2파산, 3인 1탈출/2파산, 2인 1탈출/1파산 종료. C_JOKER 빨간색 강조, 원카드! 버튼 자동 비활성화. (2) 포커 참가비(Ante) 및 올인(0별) 지원, DeltaStars 표시. (3) 도둑잡기 선택→버튼 뽑기 UX. (4) 블랙잭 PVP 레이드: 보스 레이드 협동 모드, 전원 준비. (5) 상대적 좌석 배치, 다크 모드 최적화, 모바일 반응형(오목 95vw) | 완료 |
| **Phase 9** | **아레나 분리 및 자동화 시스템** | 완료 |
...
* **PVP/PVE 시스템의 물리적 분리:**
  * **로비 UI 개편:** `index.html`을 '⚔️ PVP 아레나'와 '🤖 솔로 PVE 챌린지' 섹션으로 완전 분리함.
  * **PVE 1인 가드:** `manager.go`에서 방 ID에 `_pve_`가 포함된 경우 2명 이상의 유저 입장을 원천 차단.
  * **PVP 봇 금지:** 순수 유저 대전을 위해 PVP 방에서는 `add_bot` 기능을 서버 단에서 차단함.
* **PVE 자동화 (Zero-Wait 시작):**
  * `bot.go`에 `SpawnBotsForPVE` 함수 도입.
  * 유저가 PVE 방 입장 시 서버가 해당 게임의 최대 인원(maxPlayers)을 계산하여 부족한 수만큼 즉시 봇을 자동 소환함.
* **전적(DB) 기록의 독립성 확보:**
  * PVE 방에서 발생한 승패 기록은 기존 게임 키값에 `_pve` 접미사(예: `blackjack_pve`)를 붙여 별도 관리.
  * 랭킹 시스템의 권위와 무결성 보호.
* **마작(Mahjong) 모드 세분화:**
  * **4인 정석 마작:** 136장 풀 패산 유지 (`plugin_mahjong.go`).
  * **3인 마작 (삼마):** 2만~8만을 제외한 108장 패산 전용 플러그인 신설 (`plugin_mahjong3.go`). 인원 3인 고정.

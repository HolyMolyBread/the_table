...
| 게임 | 파일 (예정) | 상태 | 비고 |
|---|---|---|---|
| 바둑 (Go) | `plugin_baduk.go` | 대기 중 | ⚫⚪ 19×19 그리드, 계가 및 패/단수 로직 |
| 체스 (Chess) | `plugin_chess.go` | 대기 중 | ♟️ 기물별 이동 가능 경로 검증 로직 |
| 장기 (Janggi) | `plugin_janggi.go` | 대기 중 | 🇰🇷 한국 장기 룰 및 기물 이동 검증 |
...
| 알까기 유니버스 | `plugin_marbles_universe.go` | 대기 중 | 💥 2D 물리 엔진(벡터/충돌) 기반 바둑알, 체스말, 장기말 알까기 |
...
| 무기 강화 시뮬레이터 | `plugin_enhance.go` | 대기 중 | ⚔️ RNG 확률 기반 강화, 파괴 연출 및 효과음 |
...
| **Phase 9.x** | **마작 (Mahjong) 엔진 구축** — 136장 패 관리, 턴 진행, 울기(치/퐁/깡) 및 점수 계산 | 예정 |
| **Phase 10.x** | **정통 보드게임 3대장** — 바둑, 체스, 장기 코어 룰 및 그리드 시스템 고도화 | 예정 |
| **Phase 11.x** | **알까기 유니버스** — 실시간 2D 물리/충돌 엔진(Canvas) 도입 및 기물별 특성 부여 | 예정 |
| **Phase 12.x** | **무기 강화 시뮬레이터** — 순수 RNG 확률 기반 뽕맛 연출 및 사운드 이펙트 적용 | 예정 |

...
| 게임 | 파일 (예정) | 상태 | 비고 |
|---|---|---|---|
| 블랙잭 | `plugin_blackjack.go` | **완료** | PVE 딜러 AI. **PVP 레이드**: 다수 플레이어가 합산된 딜러 하트를 깎는 협동 모드. **전원 준비** 시스템(인간+봇 모두 Ready 필수). 접두사: `blackjack` |
| 텍사스 홀덤 | `plugin_holdem.go` | **완료** | 별(⭐)×10 서바이벌. **참가비(Ante)** 라운드당 별 1개. **올인(0별)** 시 해당 라운드 끝까지 카드 지급·승부 가능. 쇼다운 DeltaStars 표시. 접두사: `holdem` |
| 세븐 포커 | `plugin_sevenpoker.go` | **완료** | 3~7구 분배, 히든 카드. **참가비(Ante)** 라운드당 별 1개. **올인(0별)** 시 해당 라운드 끝까지 카드 지급·승부 가능. 접두사: `sevenpoker` |
| 인디언 포커 | `plugin_indian.go` | **완료** | 하트 서바이벌(❤️×10), 개별 상태 전송(뒷면/앞면), 선공/포기 -1, 승부/패 -2+2, 접두사: `indian` |
| 도둑잡기 | `plugin_thief.go` | **완료** | 2~4인, 53장(52+조커) 페어 제거. **선택→버튼 뽑기 UX** (카드 선택 후 🃏 카드 뽑기 버튼 클릭). 접두사: `thief` |
| 원카드 | `plugin_onecard.go` | **완료** | 2~4인, 탑 카드 매칭, J=스킵 Q=방향반전. **서바이벌 룰**: 4인 2탈출/2파산, 3인 1탈출/2파산, 2인 1탈출/1파산 시 종료. C_JOKER 빨간색 강조, 원카드! 버튼 자동 비활성화. 접두사: `onecard` |

### 🀄 타일/그리드 게임

| 게임 | 파일 (예정) | 상태 | 비고 |
|---|---|---|---|
| 마작 | `plugin_mahjong.go` | **Phase 1 완료** | 🀄 리치 마작 기반. 패 분배·쯔모·타패 구현. **4인 필수** (2~3인 모드 향후 예정). 치/퐁/깡·역 계산 대기 중. 접두사: `mahjong` |
